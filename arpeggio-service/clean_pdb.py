...
''', formatter_class=argparse.RawTextHelpFormatter)
    ...
    parser.add_argument(
        'pdb', type=str, help='Path to the PDB file to be cleaned.')
    parser.add_argument('-rmw', '--remove-waters',
                        action='store_true', help='Remove waters.')
    parser.add_argument('-kh', '--keep-hydrogens',
                        action='store_true', help='Keep hydrogens.')
    parser.add_argument('-if', '--informative_filenames', action='store_true',
                        help='Keep a record of the flags used for cleaning the output filename.')
    ...
    structure = pdb_parser.get_structure(os.path.split(
        os.path.splitext(pdb_path)[0])[1], pdb_path)
    ...
    polypeptide_chain_id_sets = [
        set([k.get_parent().id for k in x]) for x in polypeptides]
    ...
    for e, polypeptide_chain_id_set in enumerate(polypeptide_chain_id_sets):
        ...
        for polypeptide_chain_id in polypeptide_chain_id_set:
            ...
            chain_polypeptides[polypeptide_chain_id] = chain_polypeptides[polypeptide_chain_id] + [
                polypeptides[e]]
    ...
    all_chain_break_residues = reduce(
        operator.add, chain_break_residues.values())
        ...
        for residue in model.get_residues():
            ...
            for atom in residue.child_list:
                ...
                output_line = PDB_LINE_TEMPLATE.format(record=record,
                                                       ...
                                                       resnum=residue.get_id()[
                                                           1],
                                                       icode=residue.get_id()[
                                                           2],
    ...
    with open('.'.join((pdb_noext, pdb_ext, 'breaks')), 'w') as fo, \
        ...
        for chain_break_residue in all_chain_break_residues:
            ...
            fo2.write('{},{}{}`{}\n'.format(chain_break_residue.get_parent().id,
                                            # RESIDUE NUMBER
                                            chain_break_residue.get_id()[1],
                                            # INSERTION CODE
                                            chain_break_residue.get_id()[
                2].strip(),
            ...
            if 'CA' in chain_break_residue.child_dict:
                ...
                raise ValueError('Chain break residue {} had no mainchain atom to extract coordinates from.'.format(
                    chain_break_residue))
